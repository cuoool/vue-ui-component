...
export default {
  ...
  render(h) {
    return <table class="ui-table">
      <thead>
        {(() => this.$slots.thead !== undefined ? this.$slots.thead :
            <tr>
              {(() => {
                if (this.showCheckbox) {
                ...
                return (
                  <th key={key} style={this.showColumnStyle(column)}>
                    <div class="flex items-center">
                      <span class="flex-grow">{this.showColumnName(column)}</span>
                    </div>
                  </th>
                )
              })}
        ...
        {(() => {
          if (this.rowGroup) {
              ...
              return [
                <tr>
                  <td class="row-group-def" colspan={this.columnLength}>
                    <a href="#" vOn:click_prevent={items.expand = !items.expand} class="mr-1">
                      <i class="far" class={items.expand ? 'fa-minus-square' : 'fa-plus-square'}></i>
                    </a>
                    <span>{key}</span>
                  </td>
                </tr>,
            ...
            return this.dataTable.map((record, index) => {
              return [
                <tr key={this.keyName !== null ? record[this.keyName] : index}>
                  {(() => {
                    if (this.showCheckbox) {
                  ...
                  {(() => {
                    if (this.expandable) {
                      return <td class="expandable-row" style="width: 40px;">
                        <div
                          class="expandable-row__icon"
                          {...{ class: { 'expandable-row__expanded': this.expandableItems[index] } }}
                ...
                (() => {
                  if (this.expandableItems[index]) {
                    return <tr>
                      <td colspan={this.columnLength} class="expandable-column">
                        {
